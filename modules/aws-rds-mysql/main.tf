...
resource "aws_iam_role" "lambda_role" {
  name = "${var.name_prefix}-mysql-va-config-lambda-role"
...
resource "aws_secretsmanager_secret" "mysql_credentials" {
  ...
  description = "Secret for mysql credentials ${var.db_host}"
  tags        = var.tags
}
...
resource "aws_iam_policy" "lambda_policy" {
  name        = "${var.name_prefix}-mysql-va-config-lambda-policy"
  description = "Policy for MySQL VA configuration Lambda function"
...
resource "aws_security_group" "lambda_sg" {
  name        = "${var.name_prefix}-mysql-va-config-lambda-sg"
  description = "Security group for MySQL VA configuration Lambda function"
  vpc_id      = var.vpc_id
...
# Add ingress rule to MySQL RDS security group to allow Lambda access
resource "aws_security_group_rule" "mysql_allow_lambda" {
  type                     = "ingress"
  ...
  description              = "Allow Lambda function to connect to MySQL for VA configuration"
}

# Create Lambda function for VA configuration
resource "aws_lambda_function" "va_config_lambda" {
  function_name = "${var.name_prefix}-mysql-va-config"
