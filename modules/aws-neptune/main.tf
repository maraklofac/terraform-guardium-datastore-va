...
locals {
  ...
  secret_name = "${var.name_prefix}-neptune-va-password"
...
resource "aws_iam_role" "lambda_role" {
  name = "${var.name_prefix}-neptune-va-config-lambda-role"
...
resource "aws_secretsmanager_secret" "neptune_credentials" {
  name        = local.secret_name
  description = "Secret for Neptune credentials ${var.neptune_cluster_identifier}"
  tags        = var.tags
}


resource "aws_secretsmanager_secret_version" "neptune_credentials_version" {
  secret_id = aws_secretsmanager_secret.neptune_credentials.id
  secret_string = jsonencode({
    username           = var.db_username
    password           = var.db_password
    endpoint           = var.neptune_cluster_endpoint
...
resource "aws_iam_policy" "lambda_policy" {
  name        = "${var.name_prefix}-neptune-va-config-lambda-policy"
  description = "Policy for Neptune VA configuration Lambda function"
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Effect = "Allow"
        Resource = [
          aws_secretsmanager_secret.neptune_credentials.arn,
        ]
      },
      {
        Action = [
        ...
        Effect   = "Allow"
        Resource = "arn:aws:neptune-db:${var.aws_region}:${local.aws_account_id}:${var.neptune_cluster_identifier}/*"
      }
    ]
  })
}
...
resource "aws_security_group" "lambda_sg" {
  name        = "${var.name_prefix}-neptune-va-config-lambda-sg"
  description = "Security group for Neptune VA configuration Lambda function"
  vpc_id      = var.vpc_id
...
resource "aws_security_group_rule" "neptune_ingress_from_lambda" {
  ...
  description              = "Allow Lambda to connect to Neptune for VA configuration"
}

# Create Lambda function for VA configuration
resource "aws_lambda_function" "va_config_lambda" {
  function_name = "${var.name_prefix}-neptune-va-config"
  ...
  environment {
    variables = {
      SECRETS_MANAGER_SECRET_ID = aws_secretsmanager_secret.neptune_credentials.id
