...
resource "aws_iam_role" "lambda_role" {
  name = "${var.name_prefix}-mariadb-va-config-lambda-role"
...
resource "aws_secretsmanager_secret" "postgres_credentials" {
  ...
  description = "Secret for mariadb credentials ${var.db_host}"
  tags        = var.tags
}
...
resource "aws_iam_policy" "lambda_policy" {
  name        = "${var.name_prefix}-mariadb-va-config-lambda-policy"
  description = "Policy for Maria DB VA configuration Lambda function"
...
resource "aws_security_group" "lambda_sg" {
  name        = "${var.name_prefix}-mariadb-va-config-lambda-sg"
  description = "Security group for Maria DB VA configuration Lambda function"
  vpc_id      = var.vpc_id
...
# Security group rule to allow Lambda to connect to MariaDB
resource "aws_security_group_rule" "mariadb_allow_lambda" {
  type                     = "ingress"
  ...
  description              = "Allow Lambda to connect to MariaDB for VA configuration"
}

# Create Lambda function for VA configuration
resource "aws_lambda_function" "va_config_lambda" {
  function_name = "${var.name_prefix}-mariadb-va-config"
