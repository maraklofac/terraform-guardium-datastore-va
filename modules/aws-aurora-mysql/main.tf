...
# Aurora MySQL VA Config Module - Main Configuration
...
locals {
  ...
  secret_name = "${var.name_prefix}-aurora-mysql-va-password"
...
resource "aws_iam_role" "lambda_role" {
  name = "${var.name_prefix}-aurora-mysql-va-config-lambda-role"
...
resource "aws_secretsmanager_secret" "aurora_mysql_credentials" {
  name                           = local.secret_name
  description                    = "Secret for Aurora MySQL credentials ${var.db_name}"
...
resource "aws_secretsmanager_secret_version" "aurora_mysql_credentials_version" {
  secret_id = aws_secretsmanager_secret.aurora_mysql_credentials.id
...
resource "aws_iam_policy" "lambda_policy" {
  name        = "${var.name_prefix}-aurora-mysql-va-config-lambda-policy"
  description = "Policy for Aurora MySQL VA configuration Lambda function"
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Effect = "Allow"
        Resource = [
          aws_secretsmanager_secret.aurora_mysql_credentials.arn,
        ]
      }
    ]
  })
...
resource "aws_security_group" "lambda_sg" {
  name        = "${var.name_prefix}-aurora-mysql-va-config-lambda-sg"
  description = "Security group for Aurora MySQL VA configuration Lambda function"
  vpc_id      = var.vpc_id
...
# Security group rule to allow Lambda to connect to Aurora MySQL
resource "aws_security_group_rule" "aurora_allow_lambda" {
  ...
  description              = "Allow Lambda to connect to Aurora MySQL for VA configuration"
}

# Create Lambda function for VA configuration
resource "aws_lambda_function" "va_config_lambda" {
  function_name = "${var.name_prefix}-aurora-mysql-va-config"
  ...
  environment {
    variables = {
      SECRETS_MANAGER_SECRET_ID = aws_secretsmanager_secret.aurora_mysql_credentials.id
