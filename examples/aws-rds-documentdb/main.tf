...
module "documentdb_va_config" {
  ...
  #----------------------------------------
  # Pass the database connection details from the documentdb module
  db_host     = var.db_host
  db_port     = var.db_port
...
locals {
  rds_documentdb_config = templatefile("${path.module}/templates/documentdb_datasource.tpl", {
    ...
    region                          = var.region
    secret_name                     = module.documentdb_va_config.documentdb_credentials_secret_name
  ...
  rds_documentdb_config_json_decoded = jsondecode(local.rds_documentdb_config)
  rds_documentdb_config_json_encoded = jsonencode(local.rds_documentdb_config)
}

#------------------------------------------------------------------------------
# Step 2: Connect the DocumentDB cluster to Guardium Data Protection (GDP)
#------------------------------------------------------------------------------
module "documentdb_gdp_connection" {
  count  = var.enable_vulnerability_assessment ? 1 : 0
  source = "IBM/gdp/guardium//modules/connect-datasource-to-va"

  datasource_payload = local.rds_documentdb_config_json_encoded
  ...
  depends_on = [module.documentdb_va_config]
}