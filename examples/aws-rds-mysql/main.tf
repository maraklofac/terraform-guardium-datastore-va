...
#------------------------------------------------------------------------------
# Step 1: Configure Vulnerability Assessment (VA) on the MySQL database
#------------------------------------------------------------------------------
module "mysql_va_config" {
  ...
  #----------------------------------------
  # Pass the database connection details from the mysql module
  db_host     = var.db_host
  db_port     = var.db_port
...
locals {
  rds_mysql_config = templatefile("${path.module}/templates/mysql_datasource.tpl", {
  ...
  rds_mysql_config_json_decoded = jsondecode(local.rds_mysql_config)
  rds_mysql_config_json_encoded = jsonencode(local.rds_mysql_config)
}

#------------------------------------------------------------------------------
# Step 2: Connect the MySQL database to Guardium Data Protection (GDP)
#------------------------------------------------------------------------------
module "mysql_gdp_connection" {
  ...
  datasource_payload = local.rds_mysql_config_json_encoded
