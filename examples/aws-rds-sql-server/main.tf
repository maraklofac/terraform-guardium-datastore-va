...
# AWS RDS SQL Server with VA Example - Main Configuration
...
#------------------------------------------------------------------------------
# Step 2: Create datasource configuration template
#------------------------------------------------------------------------------
locals {
  rds_mssql_config = templatefile("${path.module}/templates/mssql_datasource.tpl", {
  ...
  rds_mssql_config_json_decoded = jsondecode(local.rds_mssql_config)
  rds_mssql_config_json_encoded = jsonencode(local.rds_mssql_config)
}

#------------------------------------------------------------------------------
# Step 3: Connect the SQL Server database to Guardium Data Protection (GDP)
#------------------------------------------------------------------------------
module "mssql_gdp_connection" {
  ...
  datasource_payload = local.rds_mssql_config_json_encoded
  ...
  # Depends on the credentials being stored
